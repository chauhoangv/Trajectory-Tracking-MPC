...
public:
    ...
    void controlLoop(const ros::TimerEvent&) {
        ...
        if (blind_step > 0 && blind_step < delta_plan.size()) {
            ...
            ROS_INFO(">>> [CHAY MU %d/%d] Steer: %.2f deg (Tu chay khong do Map)", blind_step + 1, (int)delta_plan.size(), steerDeg);
        ...
        // PHÂN LOẠI ĐỊA HÌNH VÀ SỐ LƯỢNG KẾ HOẠCH
        if (total_yaw_diff > 0.9) { 
            gamma = 0.8;               
            steps = 2;                 
            look_ahead_val = 5;       // Cua gắt chỉ tính kế hoạch 2 bước gần để không leo lề
            ROS_INFO_THROTTLE(0.5, "DANG CONG GAT (Total Diff: %.2f)", total_yaw_diff);
        } 
        else if (total_yaw_diff > 0.5) { 
            gamma = 0.6; 
            steps = 3;                // Cua nhẹ, nhắm mắt chạy nháp 3 nhịp
            look_ahead_val = 7;
            ROS_INFO_THROTTLE(0.5, "DANG VAO CUA (Total Diff: %.2f)", total_yaw_diff);
        }
        else { 
            gamma = 0.4; 
            steps = 5;                // ĐƯỜNG THẲNG: Tính 1 mạch 5 mốc, chạy nhắm mắt 5 nhịp (0.5s)
            look_ahead_val = 10;       
        }
        ...
        if (error_current > 0.20) {
            look_ahead_val = 15; 
            gamma = 0.5;        
            steps = 2;                // Xa tâm quá thì mở mắt thường xuyên lại để đo
        }

        // Tạo lại mảng trắng độ dài bằng đúng số step yêu cầu của địa hình
        if ((int)delta_plan.size() != steps) delta_plan.assign(steps, 0.0);

        ROS_INFO("1. MO MAT DO MAP | X: %.2f | Y: %.2f | Tinh Ke hoach %d steps", x_real, y_real, steps);

        // MIT Adaptive Control
        for (int iter = 0; iter < 50; ++iter) {
            ...
            // Nhẩm tính các điểm tương lai liên tiếp
            for(int s=0; s<steps; ++s) {
                ...
                double pred_dt = dist_to_target / std::max(0.1, v);

                double dt_step = pred_dt;
                ...
                double dx_err = xp[s] - ref_xf;
                double dy_err = yp[s] - ref_yf;
                ...
                // CHẾ ĐỘ CHẠY OPEN-LOOP (MÙ):
                // Ở chế độ này, mỗi biến delta_plan[s] tự sửa lỗi riêng mình thay vì dội ngược!
                // Mốc số 0 phụ trách khúc đầu, Mốc số 1 phụ trách khúc giữa, Mốc số 2 phụ trách khúc cuối.
        ...
        // Khởi động đếm số bước để bắt đầu cho xe nhắm mắt chạy thả dốc
        blind_step = 1;
        if (blind_step >= delta_plan.size()) {
            blind_step = 0; 
        }
