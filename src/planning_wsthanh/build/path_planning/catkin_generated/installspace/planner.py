...
def find_nearest_node(G, x, y):
    ...
    for node, data in G.nodes(data=True):
        ...
        if dist < min_dist:
            min_dist = dist
            nearest_node = str(node) # Đảm bảo node luôn là chuỗi
...
def run_planner():
    ...
    # Đã sửa sang topic filtered để lấy tọa độ chính xác từ EKF
    rospy.Subscriber('/odometry/filtered', Odometry, odom_callback)
    
    # 1. Lấy tọa độ mặc định từ file localization.yaml ngay lập tức
    ...
    # ÉP PYTHON CHỜ 0.5 GIÂY ĐỂ ỔN ĐỊNH
    rospy.sleep(0.5)
    ...
    # blue_nodes = ['n368', 'n386', 'n327', 'n261' ]
    #vòng xuyến:
    # blue_nodes = ['n331', 'n335', 'n341', 'n332' ]
    ...
    if path_result:
        ...
        rospy.sleep(0.5) # Đợi nhịp publisher đăng ký vào master
