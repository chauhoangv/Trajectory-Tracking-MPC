# FILE VẼ BIỂU ĐỒ MINH HOẠ CHO MÔ HÌNH XE ĐẠP
...
def visualize_academic_proof():
    ...
    if df.empty: return
    # 1. Xác định Chu kỳ đích (Latest Cycle - Lấy chu kỳ replan gần nhất có dự báo cũ vượt hành lang)
    plan_exists = df[df['type'] == 'FINAL_PLAN']
    if plan_exists.empty: return
    
    planning_df = df[df['type'] == 'PLANNING']
    replan_cycles = sorted(plan_exists['cycle_id'].unique(), reverse=True)
    target_cycle = None
    for cycle in replan_cycles:
        # Lấy dự báo ở vòng lặp đầu tiên (iter = 0) sử dụng góc lái cũ trước khi replan
        init_plan = planning_df[(planning_df['cycle_id'] == cycle) & (planning_df['iter'] == 0)]
        if not init_plan.empty:
            # e_lat > 1.0 nghĩa là sai số ngang vượt quá 0.20m (tức là ra ngoài hành lang 40cm)
            if (init_plan['e_lat'] > 1.0).any():
                target_cycle = cycle
                break
    ...
    # (Đã loại bỏ hoàn toàn việc vẽ quỹ đạo của các chu kỳ quá khứ theo yêu cầu để tránh các nét mờ thừa ở đuôi xe)
