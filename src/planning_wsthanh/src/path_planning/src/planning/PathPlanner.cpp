...
PathPlanner::PathPlanner(double vref, int N, double T) : track(), spline_utils(), path_utils(), vref(vref), N(N), T(0.1) {
...
void PathPlanner::construct_path(bool use_gps) {
  ...
  // --- IN DỮ LIỆU CHỨNG MINH ---
  std::string file_path = "/home/hoangchau/Documents/quy_dao.csv";
  ...
  if (csv_file.is_open()) {
      csv_file << "x,y,khoang_cach_d_met\n"; // Header file CSV
    
      for (size_t i = 0; i < condensed_path.size(); ++i) {
          double d = 0;
          // Tính khoảng cách Euclide giữa điểm hiện tại và điểm kế tiếp
          if (i < condensed_path.size() - 1) {
              d = std::sqrt(std::pow(condensed_path[i+1].x - condensed_path[i].x, 2) +
                            std::pow(condensed_path[i+1].y - condensed_path[i].y, 2));
          }
        
          // Ghi dữ liệu: Tọa độ X, Tọa độ Y, Khoảng cách d
      ...
      csv_file.close();
      std::cout << "Da xuat file" << std::endl;
  }
  // ------------------------------------------
}
